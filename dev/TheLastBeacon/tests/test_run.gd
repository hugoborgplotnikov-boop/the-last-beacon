...
## accumulation and application, shards, lap scaling, and the run restart on
## death (shards survive). Deterministic: logic-level checks + arena
...
func _initialize() -> void:
	...
	run.apply_card("heros_vigor")
	h.check(run.buffs.get("damage") == 1 and run.buffs.get("hp") == 1, "buffs recorded")
	# Shards accumulate per victory.
	run.record_victory()
	h.check(run.shards == 3, "victory grants 3 shards")
	print("TEST run: pool+draws+buffs+shards ok — spawning arenas")
...
func _physics_process(_delta: float) -> bool:
	...
	match frames:
		10:
			# The arena applies the run state: hero buffs + boss lap scaling.
			...
			h.check(player1.attack_damage == 2, "hero's damage buff applied (+1 -> %d)" % player1.attack_damage)
			h.check(player1.max_health == 6, "hero's vigor applied (+1 -> %d)" % player1.max_health)
		...
		40:
			# Death ends the run and restarts it fresh; shards survive.
		...
		300:
			...
			h.check(run.shards == 3, "shards survived the death")
